...
bestParameters <- read.csv(file.path(resultsDataFolder, "SVMParameters.csv"))
...
gridSVMpredict <- numeric(nrow(grid))
gridMRFpredict <- numeric(nrow(grid))
gridChanged <- numeric(nrow(grid))
gridELocal <- numeric(nrow(grid))
phenNum <- 1
...
for (p in 6:ncol(dat)){
  ...
  for (t in 1:it_lim) {
    ...
    if (dU < 0) {
      ...
      # Get the neighbours
      adjInd <- which(adj[rind,] != 0)
    
      # Update local energy
      E_local[rind] <- E_local_new
      for (z in 1:length(adjInd)) {
        w <- adjInd[z]
        U_sp <- sum(adj[w,][MRF_pred != MRF_pred[[w]]] * beta)
        E_local[w] <- U_ling[w, MRF_pred[[w]]] + U_sp
      }
      ...
      # Probability of failure, in dependency to temperature
      prob <- exp(-dU/temp)
...
p <- ggplot(data=globalEnergies, aes(x=Iteration, y=Energy, color = Phenomena, group = Phenomena)) +
  theme_classic() + 
  geom_line()
