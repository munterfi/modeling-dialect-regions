...
phens <- colnames(dat)
# IMPORTANT!!! IF ORIGINAL FILESIZE UNCOMmENT LINES BELOW
phens[phens=="iv3es_ihm"] <- "iv3_es_ihm"
phens[phens=="iv3ihm_es"] <- "iv3_ihm_es"
colnames(dat) <- phens
...
for (i in 1:nrow(coordsBFS)) {
  ...
  # IMPORTANT!!! IF ORIGINAL FILESIZE UNCOMNMENT LINES BELOW
  colSums(pick[3:211], na.rm = TRUE, dims = 1)
  #colSums(pick[3:23], na.rm = TRUE, dims = 1)
  # IMPORTANT!!! IF ORIGINAL FILESIZE UNCOMmENT LINES BELOW
  datSample <- rbind(datSample, colSums(pick[3:211], na.rm = TRUE, dims = 1))
  #datSample <- rbind(datSample, colSums(pick[3:23], na.rm = TRUE, dims = 1))
}
# IMPORTANT!!! IF ORIGINAL FILESIZE UNCOMmENT LINES BELOW
colnames(datSample) <- phens[3:211]
#colnames(datSample) <- phens[3:23]
...
phenNames <- phenNames[3:length(phenNames)]
...
# Remove phenomenas which only show one variant or less than 90% variation
n <- nrow(dat)
varLimit <- n * 0.98
toRemove <- character()
toRemoveReason <- character()
for (i in 6:ncol(dat)){
  # Check for levels
  if (length(levels(dat[ ,i]))<=1) {
    toRemove <- c(toRemove, names[i])
    toRemoveReason <- c(toRemoveReason, "No variance")
    # cat(names[i])
    # cat(" removed - no variance\n")
  } else {
    # Check for enough variance
    test <- summary(dat[,i])
    for (j in 1:length(test)){
      if(test[j]>varLimit){
            toRemove <- c(toRemove, names[i])
            toRemoveReason <- c(toRemoveReason, "Too few variance")
            # cat(names[i])
            # cat(" removed - to few variance\n")
            next
      }
    }
  }
...
dat[toRemove] <- NULL
names <- colnames(dat)
phenNames <- names[6:length(names)]

# Prepare table
removed <- data.frame(toRemove, toRemoveReason)
colnames(removed) <- c("Phenomena", "Reason")

# Save process data as csv file
write.csv(dat, file = file.path(resultsDataFolder, "randomSampling.csv"))
write.csv(removed, file = file.path(resultsDataFolder, "removedPhenomena.csv"))
```
...
outline <- readOGR(geoDataFolder, "outlineCH", verbose=FALSE)
proj4string(outline) <- CH1903
outline <- fortify(outline)
outlinecol <- "dimgray"

# Load CH oultine and the multiplot function
lakes <- readOGR(geoDataFolder, "Lakes_dissolve_simplify", verbose=FALSE)
proj4string(lakes) <- CH1903
lakes <- fortify(lakes)
lakecol <- "mediumblue"
...
# Load STATPOP13B_100m & cutoff last column
...
p <- qplot() +
  ...
  # + ggtitle("Overview Plot")

print(p)
```

```{r Overview plot 2, fig.align='center', echo=FALSE, fig.cap="Overview plot of the communities for phenomena iv17. The point size represents the number of participants per survey site."}
p <- qplot() +
  coord_fixed(ratio = 1) +
  xlab("Easting") +
  ylab("Northing") +
  ...
  geom_polygon(data = lakes,
          ...
          fill = lakecol, size = .2, alpha=0.3) +
  geom_point(data=dat, aes(x = Easting, y = Northing,
           size=Count, colour=iv17), alpha=0.8) +
  geom_path(data = outline,
            aes(x = long, y = lat, group = group, size = Population),
            color = outlinecol, size = .2)
  # + ggtitle("Overview Plot")
...
knitr::kable(head(dat[ ,1:14],5), caption = "Structure of the processed data set (random sampling & majority vote).")
```
...
```{r Removed table, echo=FALSE}
removed <- read.csv(file.path(resultsDataFolder, "removedPhenomena.csv")) #, header = T)
removed$X <- NULL

knitr::kable(removed, caption = "Removed phenomenas, due to missing variance.")
```
...
```{r Setup grid, fig.align='center', echo=FALSE, message=FALSE, message=FALSE, fig.cap="Regular grid expanded over the area of Switzerland."}
# Create grid of the size of CH
gridRes <- 5000 # Resolution in meters
gridRange <- apply(outline[c("long", "lat")], 2, range)
...
knitr::kable(round(gridInfo,1), caption = "The extent and resolution of the expanded grid.")
...
#              aes(Easting, Northing), size=0.7, shape=3, alpha = 0.6) +
#   geom_path(data = outline,
#           aes(x = long, y = lat, group = group),
#           color = outlinecol, size = .2)
# print(p)
...
# Grid specfic data table
#knitr::kable(round(head(as.data.frame(grid[order(-grid$Population),]),3),1), caption = "Overview of the #processed grid data points.", row.names = FALSE)
```
...
# Apply Toblers function
#nn$W <- sapply(X=nn$m, FUN = paths)
...
for (i in 1:nrow(grid.nn.i)){
  ...
  sector <- crop(dhm100, c(from[1]-factor*gridRes, from[1]+factor*gridRes,
                           from[2]-factor*gridRes, from[2]+factor*gridRes))
...
# Save nn as csv file
write.csv(nn2, file = file.path(tmpDataFolder, "nn.csv"))
```
...
levelplot(apply(t(log(adj[1000:1200, 1000:1200]+1)),1,rev), scales=list(draw=TRUE), colorkey=TRUE,
...
# Put all together
results <- cbind(as.data.frame(grid)[c("Easting", "Northing", "Height", "Population")], gridSVMpredict, gridMRFpredict, gridChanged)
...
plotPhen <- function(p) {
  g <- ggplot() +
    ...
    geom_point(data =  grid, 
               aes(Easting, Northing, colour = eval(parse(text = paste("grid$", p, "_MRF", sep="")))), 
               size=1.9, alpha=0.5, shape=15) +
    geom_point(data = grid[eval(parse(text = paste("grid$", p, "_Change", sep="")))==1,], 
               aes(Easting, Northing), size=2, shape=0) +
